...
function bestYearAvg(arr) {
  ...
  const grouped = [];
  let slicer = 0;
  //stores movies with same year in an array in grouped
  for (i = 1; i < sorted.length; i++) {
    if (sorted[i].year !== sorted[i - 1].year) {
      grouped.push(sorted.slice(slicer, i));
      slicer = i;
    }
  }
  //reduces each year array into an object with 2 properties: year, score
  for (let i = 0; i < grouped.length; i++) {
    grouped[i] = {
      year: grouped[i][0].year,
      score:
        grouped[i].reduce((score, movie) => score + movie.score, 0) /
        grouped[i].length
    };
  }
  //sorts the array, and leaves only the highest scoring element
  grouped.sort((a, b) => b.score - a.score).splice(1);
  return `The best year was ${grouped[0].year} with an average score of ${grouped[0].score}`;
}
