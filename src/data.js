...
function getAllDirectors(moviesArray) {
  const director = moviesArray.map((item) => item.director);
  return director;
}
console.log(getAllDirectors(movies));

let allDirectors = getAllDirectors(movies);
let allDirectorsNoRepetition = allDirectors.filter((element, index) => {
  return allDirectors.indexOf(element) === index;
});
console.log(allDirectorsNoRepetition);

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
function howManyMovies(moviesArray) {
  const directorAndGenre = moviesArray.filter((film) => {
...
function scoresAverage(moviesArray) {
  ...
  let averageScores =
    moviesArray.reduce((sum, { score }) => sum + (score ? score : 0), 0) /
    moviesArray.length;

  let roundedAverageScore = Math.round(averageScores * 100) / 100;
...
function dramaMoviesScore(moviesArray) {
  ...
  let averageScore =
    dramaScore.reduce((sum, { score }) => sum + (score ? score : 0), 0) /
    dramaScore.length;
...
function orderByYear(moviesArray) {
  ...
  // Sort the movies array based on year and title
...
function orderAlphabetically(moviesArray) {
  const movieTitlesSorted = moviesArray.map((movie) => movie.title).sort();
  if (movieTitlesSorted.length > 20) {
    return movieTitlesSorted.slice(0, 20);
  } else {
...
function turnHoursToMinutes(moviesArray) {
  const result = moviesArray.map((movie) => ({ ...movie }));
  const totalMinutesArray = result.map((movie) => {
