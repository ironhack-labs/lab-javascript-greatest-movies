...
const getAllDirectors = movies.map(function(moviesArray) {
  return moviesArray.director;
});
console.log(getAllDirectors)

const allDirectorsNoDuplicate = getAllDirectors.filter((element, index) => {
  return getAllDirectors.indexOf(element) === index;
})
console.log(allDirectorsNoDuplicate)


// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
const howManyMovies = movies.filter(function (moviesArray){
  // const dramaMovie = movies.genre.filter(filme => filme.genre.includes("Drama"))
  const directorMovie = moviesArray.director === "Steven Spielberg" && moviesArray.genre.includes("Drama")
  return directorMovie;
})
console.log(JSON.stringify(howManyMovies));

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
const scoresAverage = movies.reduce((sum, {score}) =>sum + score, 0) / movies.length 
const scoreAverageRounded = Math.round(scoresAverage * 100) / 100

console.log(scoreAverageRounded);
...
const dramaMoviesScore = movies.filter(movie => movie.genre.map(genre => genre.toLowerCase()).includes("drama"));
   const averageScore = dramaMoviesScore.reduce((acc, movie) => acc + movie.score, 0) / dramaMoviesScore.length;
