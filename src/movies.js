...
function howManyMovies(moviesArray) {
  // warning: example is "drama" but specs expect "Drama"
  return moviesArray.filter(movie => movie.director == "Steven Spielberg" && movie.genre.indexOf("Drama") > -1).length
}
...
function scoresAverage(moviesArray) {
  ...
  return Math.round(moviesArray.reduce(
      (accumulator, movie) => {
        let score2 = movie.score
        if (movie.score == undefined || movie.score == "") score2 = 0
        return accumulator + score2
      }
      , 0
    ) / moviesArray.length * 100) / 100 // it's tempting to use ternary operator again here because if len = 0, then we have 0/0, which could be replaced by 0/1 to return 0...
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(moviesArray) {
  return scoresAverage(moviesArray.filter(movie => movie.genre == "Drama"))
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(moviesArray) {
  // const moviesArray2 = moviesArray.slice() // shallow copy because .sort() method sorts in-place
  const moviesArray2 = [...moviesArray]
  moviesArray2.sort(function(movie1, movie2) { // overrinding default compare function
    // implementing new compare function based on specs
    if (movie1.year < movie2.year) return -1;
    if (movie1.year > movie2.year) return 1;
    if (movie1.year === movie2.year) {
      if (movie1.title < movie2.title) return -1
      if (movie1.title > movie2.title) return 1
      if (movie1.title === movie2.title) return 0 // probably the same entries, not my problem!
    }
  })
  return moviesArray2
}
...
function orderAlphabetically(moviesArray) {
  ...
  titlesArray2.sort(function(title1, title2) { // overrinding default compare function
    // implementing new compare function based on specs
...
function turnHoursToMinutes(moviesArray) {
  const moviesArray2 = moviesArray.map(function(movie) {
    ...
    // but that's also a shallow copy of the array: for its object elements, only the reference is copied, not the object's content itself
    // using spread syntax copy = {...object} allows to actually copy the object properties, but nested object will still get copied by reference

    // let hours = movie.duration.slice(0, 1)
    // let min = movie.duration.slice(3, -3)
    // movie.duration = Number(hours) * 60 + Number(min) // NO! this still pass the original movie object!
    // let movie2 = {...movie, duration: Number(hours) * 60 + Number(min)} // OK!
    // return movie2
    // // WORKS FOR "0h 31min" BUT WON'T WORK FOR "10h 31min" OR "1h 1min"

    // let movie2 = {...movie}
    // let hours = movie2.duration.split("h ")[0]
    // let min = movie2.duration.split("h ")[1].slice(0, -3)
    // movie2.duration = Number(hours) * 60 + Number(min)
    // return movie2
    // // WORKS FOR "0h 31min" "10h 31min" "1h 1min" BUT NOT FOR "1min"

    // // in case there is no "h" (so only "min") or no "min" (so only "h")
    // if (movie.duration.indexOf(" ") == -1)
    // if      (!movie.duration.includes(" ") && movie.duration.indexOf("min") == -1) movie.duration = movie.duration + " "
    // else if (!movie.duration.includes(" ") && movie.duration.indexOf("h")   == -1) movie.duration = " " + movie.duration
    // const [hours, min] = movie.duration.split(" ") // split() returns a shallow copy array then destructuring it into [hours, min]
    // console.log(hours + " " + min)
    // movie.duration = Number(hours.slice(0, hours.length - "h".length)) * 60 + Number(min.slice(0, min.length - "min".length)) // end is not included in slice, so to extract 1 item: slice(i, i+1)
    // movie.duration = Number(hours.slice(0, -1)) * 60 + Number(min.slice(0, -3)) // negative index counts back from the end of the array
    // return movie
    // // WORKS BUT STILL MODIFIY THE movie OBJECTS, SO THE FUNCTION CANNOT BE CALLED TWICE WITH THE SAME ARRAY ARG

    // let movie2 = {...movie, duration: movie.duration}
    const movie2 = {...movie}
    if (movie2.duration.indexOf(" ") == -1)
    if      (!movie2.duration.includes(" ") && movie2.duration.indexOf("min") == -1) movie2.duration = movie2.duration + " "
    else if (!movie2.duration.includes(" ") && movie2.duration.indexOf("h")   == -1) movie2.duration = " " + movie2.duration
    const [hours, min] = movie2.duration.split(" ") // split() returns a shallow copy array then destructuring it into [hours, min]
    console.log(hours + " " + min)
    movie2.duration = Number(hours.slice(0, hours.length - "h".length)) * 60 + Number(min.slice(0, min.length - "min".length)) // end is not included in slice, so to extract 1 item: slice(i, i+1)
    movie2.duration = Number(hours.slice(0, -1)) * 60 + Number(min.slice(0, -3)) // negative index counts back from the end of the array
    return movie2
  })
  return moviesArray2
 }
...
/* // Jasmine does like declaring this variable which is also in data.js...
const movies = [
  {
    title: 'The Shawshank Redemption',
    year: 1994,
...
console.log(turnHoursToMinutes(movies2)[0].duration)
console.log(turnHoursToMinutes(movies2)[1].duration)

//console.log(bestYearAvg(newMoviesArr))
console.log(bestYearAvg(movies)) // 'The best year was 1972 with an average score of 9.2'
