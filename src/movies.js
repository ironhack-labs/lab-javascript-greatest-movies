...
function howManyMovies(movies) {
    const spilberg = movies.filter(movie => movie.director === 'Steven Spielberg').filter(movie => movie.genre.includes('Drama'))
    if (!spilberg){
        return 0 
    } 
...
function ratesAverage(movies) {
    ...
    const rates = movies.map((movie) => movie.rate).reduce(function(array,value){
...
function dramaMoviesRate(movies) {
    ...
    const rates = dramas.map((movie) => movie.rate)
    const ratesTotal = rates.reduce(function(arr,val) {
    ...
    return Number((ratesTotal/dramas.length).toFixed(2))
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(movies) {
    let yearlyOrder = movies.slice()
    yearlyOrder.sort((movieA, movieB) => (movieA.year > movieB.year) ? 1 : ((movieB.year > movieA.year) ? -1 : movieA.title.localeCompare(movieB.title)))                             
    return yearlyOrder;
}
