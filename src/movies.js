...
function scoresAverage(movies) {
  ...
  const decimalScore = totalScore / movies.length

  return parseFloat(decimalScore.toFixed(2));
}
...
function dramaMoviesScore(movies) {
  ...
  const dramaAverage= dramaMovies.reduce((accumulator, movie) => {
    if (movie.score === null || movie.score === undefined) {
      return accumulator;
    }
 ...
 const decimalDramaScore = dramaAverage / dramaMovies.length;

 return parseFloat(decimalDramaScore.toFixed(2));
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(movies) { 
  const movieYears = movies.map(movie => movie).sort((n1,n2) => n1.year - n2.year);
  return movieYears;
...
function orderAlphabetically(movies) {
  const alphabeticOrder = movies.map(movie => movie.title).sort();
...
function turnHoursToMinutes(movies) {
  ...
  const movieTime = movies.map((movie) => {
    
    let hourToMinute = movie.duration.split(' ');
    movie.duration = parseInt(hourToMinute[0]) * 60 + parseInt(hourToMinute[1]);
    return movie;
