...
function howManyMovies(movies){
     ...
     return (movies.filter( (movieGenre,index) =>{
          return ( (movieGenre.director === "Steven Spielberg") &&
...
function ratesAverage(arr){
    if (arr.length == 0){ 
        ...
        const ratedFilms = arr.filter((film) => {
            return film.rate > 0;
        })
        const totalScores = ratedFilms.reduce((total, movie) => {
            return total + movie.rate;
        }, 0)
        return parseFloat((totalMovieRate / arr.length).toFixed(2))
    }
}
...
dramaMoviesRate = Math.round(dramaMoviesRate)

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)

const orderByYear = (movies) =>{
     let movieSelection =  movies.sort( (a,b) => {
       if((a.year - b.year) === 0){
          return a.title.movieComparison(b.title);             
       }else{
