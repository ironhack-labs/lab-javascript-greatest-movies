...
function howManyMovies (movies) {
    ...
    movies.filter (movie => movie.genre === ‘Drama’)
    movies.map (movie => movie.stevenMovies)
    return (stevenMovies);
};
...
function ratesAverage (movies) {
    const rates = movies.reduce ((acc,stars) =>{
        return acc + stars.rate
    },0 )
    const avg = rates / movies.length
    return avg
}
...
function dramaMoviesRate (movies) {
    const dramaM =movies.filter (movie => movie.genre ===‘Drama’)
    movies.reduce ((acc,dramaStars) =>{
        return acc + dramaStars.rate
    },0 )
    const avgDrama = dramaM / 
    return avgDrama
}
...
function orderByYear(movies){
    const orderedMovies = [...movies.year].sort((a,b) => {
        return a-b
    })
...
function orderAlphabetically(movies){
    ...
    const alphOrder = [...movies.title].sort((a,b) => {
        return a-b
    })
    return alphOrder
}
