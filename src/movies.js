...
function dramaMoviesScore(moviesArray) {
    ...
    const sumOfDramas = dramas.reduce((accumulator, currentValue) => {
        return accumulator + currentValue.score;
    },0);

    return Math.round(sumOfDramas/dramas.length*100)/100;
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(moviesArray) {
    const ascendedOrder =  moviesArray.map((currentMovie) => {
        return currentMovie.year;
    }); 
    return ascendedOrder.sort();
}
