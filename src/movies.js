...
function howManyMovies(moviesArray) {
    ...
    return `Steven Spielberg directed ${spielbergMovies.length} movies`
}
...
function scoresAverage(moviesArray) {
    const averageScore = moviesArray.reduce((sum, movie) => 
              sum + movie.score, 0)
...
function dramaMoviesScore(moviesArray) {  
    ...
    const averageDramaScore = dramaMovies.reduce((sum, movie) => 
    sum + movie.score, 0)
    return (averageDramaScore / dramaMovies.length).toFixed(2)
}
