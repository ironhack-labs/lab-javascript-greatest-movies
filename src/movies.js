...
function getAllDirectors(arr) {
  let allDirectors = arr.map(function(movieDirector){
...
// Bonus 1.1:
let cleanArray = directors.filter((item, position) => {
...
function scoresAverage(array) {
  ...
  let arrayOfScores = array.map(function(movie){
    return movie.score;
  }).filter(function(score){
    return score;
  })

  let sum = arrayOfScores.reduce(function(acc, currentValue){
...
function dramaMoviesScore(array) {
  ...
  let arrayOfDramas = array.filter(function(item) {   
    return item.genre.indexOf('Drama') != -1;
  })
  if (arrayOfDramas.length === 0){
    return 0;
  } //array de objetos de filmes com genre 'Drama' com ou sem score

  let onlyDramasWithScores = arrayOfDramas.filter(function(item){
    return item.score;
  }) //verificar todos que têm score; continua sendo array de objetos

  let sum = onlyDramasWithScores.reduce(function(acc, currentValue) {
    return acc + currentValue.score;
  },0.0)
  return Math.round((sum / arrayOfDramas.length)*100)/100.0;
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(array) {
  let anotherArray = array.filter(function (item){
    return item.year;
  })
...
function orderAlphabetically(array){
  let anotherMoviesArray = array.filter(function(item){
    return item.title; 
  })
  anotherMoviesArray.sort(function (a,b){
    return a.title.localeCompare (b.title);
  })
  let slicedArray = anotherMoviesArray.slice(0, 20);
  let firstTwentyMovies = slicedArray.map(function(item){
    return item.title;
  })
