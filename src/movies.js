...
function howManyMovies(object) {
    const stevenSpielberg = movies.filter( steven => {
        if(steven.director.toLowerCase() == "steven spielberg") {
            return steven;
        }
    });
    const stevenSpielbergMovies = stevenSpielberg.length;
...
function ratesAverage(object) {
    const rates = object.reduce((accumulator, rate) => {
        console.log(`accu: ${accumulator}, rate: ${rate}`)
    }, 0); 
    
    console.log(rates)
    /*
    const avgRate = rate / object.rate.length;
    return avgRate;
    */
}
