...
function turnHoursToMinutes(moviesArray) {
    const movieCopyH = JSON.parse(JSON.stringify(moviesArray))
    const changeHour = movieCopyH.map(movies => {
        const hour = (movies.duration).split(" ")[0].replace("h", "")
        const minut = (movies.duration).split(" ")[1].recplace("min")
        return movies.duration = Number(hour) * 60 + Number(minut)
    })
    return changeHour
}
