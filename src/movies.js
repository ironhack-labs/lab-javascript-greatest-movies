const movies = [ // perdão DK e Ju, não sabia como importar essa const lá do data
  {
...
function getAllDirectors(array) {
  ...
  let directorsFiltered = allDirectors.filter(function(element, index){ //declaração da função
      return allDirectors.indexOf(element) === index; //o que elavai fazer
  })
...
function howManyMovies(moviesArray) {
  let spielbergArray = moviesArray.filter(function(movie){
    return movie.director === 'Steven Spielberg' && movie.genre.includes('Drama'); //busca o Steven no array e verifica se o genero é drama
  })
...
function scoresAverage(array) {
  const average = array.reduce(function(sum, movie){ // reduz o array para um unic valor
...
function dramaMoviesScore(array) {
  const dramaMovies = array.filter(function(movie){ //filtra os generos
    return movie.genre.includes('Drama'); // contabiliza os Dramas
  })
...
function orderByYear(array) {
  let orderedYears = array.sort(function(a, b){
    if(a.year !== b.year){  // se os anos forem diferentes
      return a.year - b.year; 
    }else if(a.year === b.year){ // se os anos forem iguais 
      let alphabet = array.sort(function(a,b){ // ordena por ondem alfabetica 
        a.title - b.title;  
      })
      return alphabet;
    }  
...
function orderAlphabetically(array) {
  let titlesArray = array.map(function(element){ // mapeia os titulos
      return element.title;
  })
  let topOrder = titlesArray.sort().slice(0,20); //exibe os 20 primeiros baseado no map
  return topOrder;
}
