...
let howManyMovies = movies.filter(function(one){
    if(one.director == "Steven Spielberg" && one.genre.includes("Drama")){
    return one;
    }
  })
...
let rates = (movies.reduce(function(acc, crr){
    return acc + crr.rate;
    },0))/movies.length
    
    console.log(rates.toFixed(2))
// Iteration 4: Drama movies - Get the average of Drama Movies
let howManyDrama = movies.filter(function(dr){
    return dr.genre.includes("Drama")
  })
  //console.log(howManyDrama)
  
  let dramaMoviesRate = (howManyDrama.reduce(function(acc,crr){
  return acc + crr.rate;
  },0))/howManyDrama.length
