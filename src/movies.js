...
function bestYearAvg(movies) {
  if (!movies.length) {
    ...
    //movies.forEach(movie => {
      //rateDividedByYear[1994].push(movie.rate)
    //})

    //for (const key in rateDividedByYear) {
      //rateDividedByYear[key] = yearAvg(rateDividedByYear[key]);
    
    

    //first same years togther and the avg of them 
  /*}
                  //[9.4, 9.0]
  function yearAvg(rates) {
    if(rates.length == 0) {
      return 0;
    }
    let newArr = movies.reduce(function(acc, currIndex) {
