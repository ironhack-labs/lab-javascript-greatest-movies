...
// Iteration 4: Drama movies - Get the average of Drama Movies
  function dramaMoviesScore(arr) {
    let filtro = arr.filter(movie => movie.genre.includes('Drama'));
    
    let sum = filtro.reduce((acc, curr) => {
      acc += curr.score
        return acc
    },0);
    
    if (sum > 0){
      let avg =  Number((sum / filtro.length).toFixed(2))
      return avg
    } else {
      return 0
    }
...
let prueba = soloTitulos.slice(0,20);
return prueba
};
