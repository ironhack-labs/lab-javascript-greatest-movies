...
//const getAllDirectors = movies.map(object => object.director);
 ...
 getAllDirectors = ()=> { return movies.map(object => object.director)}

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?

howManyMovies = () => {
    const StevenDramaArray = movies.filter(object => object.director == 'Steven Spielberg' && object.genre.includes ('Drama'));
...
ratesAverage = () => {
    ...
    const rateSum = movies.reduce (function(sum, movie){
      if (movie.rate){
...
dramaMoviesRate = () => {
    const dramaArray = movies.filter(object => object.genre.includes ('Drama'));
    const reduceDrama = dramaArray.reduce (function(sum, movie){
        if (movie.rate){
          return sum + movie.rate
        } else {
           return sum;
    ...
    return Math.round((reduceDrama / dramaArray.length) * 100) / 100
}
...
orderAlphabetically = ()=> {
    const alphaArray = movies.map(movies => movies.title);
    alphaArray.sort();
