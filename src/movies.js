...
function howManyMovies(moviesArray) {
    const stevenSpielberg= moviesArray.filter(function (movie) {
...
function dramaMoviesScore(moviesArray) {
    ...
    let moviesGenre = moviesArray.filter(function (movie) {
    ...
    return scoresAverage(moviesGenre);
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(moviesArray) {   
    const movies = moviesArray.map(function (element){
        return element;
    });
...
function orderAlphabetically(moviesArray) {
    let titles = moviesArray.map(function (element){
