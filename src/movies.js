...
function getAllDirectors(movies) {
  const onlyDir = movies.map(eachMovie => eachMovie.director)
  return onlyDir
...
function howManyMovies(array) {
  const stevenDramaMovies = array.filter(eachFilm => {
    return eachFilm.genre.includes("Drama") && eachFilm.director.startsWith("Steven")
...
function scoresAverage(movieScore) {
  const promedioScore = movieScore.reduce((acc, eachScore) => acc + eachScore, 0)
  const totalScoreAverage = promedioScore / movieScore.length;
...
function dramaMoviesScore(movieDramaScore) {

  const promedioDrama = movieDramaScore.reduce((acc, eachDramaScore) => acc + eachDramaScore, 0)
  const totalScoreDramaAverage = promedioDrama / movieDramaScore.length;
  return totalScoreDramaAverage
}

function checkScore(boolean) {
  if (totalScoreAverage < totalScoreDramaAverage)
    return true
...
function orderByYear(arrayYears) {
  const ordenAscendente = arrayYears.sort(function (a, b) { return a - b });
}
