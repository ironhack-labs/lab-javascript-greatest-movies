...
function scoresAverage(moviesArray) {
    ...
    let resultScores = moviesArray.reduce ((acc, movies) => {
        if (movies.score)  {
    ...
    return Math.round ((resultScores * 100) / moviesArray.length) / 100;
    }
...
function orderByYear(moviesArray) {
    ...
    const yearMovie = moviesArray.splice(0, moviesArray.length)
    return yearMovie.sort(function (a, b) {
        if (a.year === b.year) {
            if (a.title < b.title) {
                return -1
