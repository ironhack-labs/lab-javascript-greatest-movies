...
function howManyMovies(moviesArray) {
    const spielbergMovies = moviesArray.filter(e => {
...
function scoresAverage(moviesArray) {
    const sum = moviesArray.reduce((total, current) => {
        return (total += current.score / moviesArray.length)
    }, 0)
...
function dramaMoviesScore(moviesArray) {
    ...
    const dramaMovies = moviesArray.filter(e => {
        return e.genre.includes('Drama')
    })

    const sum = dramaMovies.reduce((total, current) => {
