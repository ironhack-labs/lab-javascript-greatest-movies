...
function getAllDirectors(moviesArray) {
    return moviesArray.map(getDirector)
}
...
function orderByYear(moviesArray) {
  let moviesArrayCopy = JSON.parse(JSON.stringify(moviesArray));//clone the array
  return moviesArrayCopy.sort(compareByYear)
}
...
function orderAlphabetically(moviesArray) {
  let moviesArrayCopy = JSON.parse(JSON.stringify(moviesArray));//clone the array
  let first20movies = moviesArrayCopy.slice(0,20);//get the 20 first array elements
  let first20titles = first20movies.map( movie => movie.title)
  return first20titles.sort()
}

// BONUS - Iteration 7: Time Format - Turn duration of the movies from hours to minutes
function turnHoursToMinutes(moviesArray) {
  // moviesArray.forEach(movie => {
  //   movie.duration.forEach((letter,index,arr) =>{
  //     console.log(letter)
  //   })
  // });
  // return moviesArray
