...
function getAllDirectors(movies) {
  const directors = movies.map (function(eachMovies) {
...
function howManyMovies(movies) {
  const steven =  movies.filter (function(eachSteven) {
...
function scoresAverage(movies) {
  const pointMovies = movies.reduce((acc, sumPoints) => {
    if (sumPoints === undefined){
    pointMoviesRaw = acc
    } else {
      pointMoviesRaw = acc + sumPoints.score
    }
    
    return pointMoviesRaw
  }, 0)

   console.log(pointMovies)  

   if(movies.length === 0){
     return 0
   }

    const average = pointMovies / movies.length
...
function dramaMoviesScore(movies) {
  dramaMovies = movies.filter ( x=> x.genre.includes('Drama')), 
    scores = dramas.map((x) => (x.score))

    total = scores.reduce((total, rating) => total + rating);

  console.log ('el total es', total);

  x = total/ scores.length
  console.log('Promedio:', x.toFixed(2))
...
function orderByYear(movies) {
  x = movies.sort(function(a, b){
      return a.year - b.year
  })
  console.log('Orden por año ascendente', x)
}


// Iteration 6: Alphabetic Order - Order by title and print the first 20 titles
function orderAlphabetically(movies) {
  aux = movies.sort(function(a, b){
    if (a.title > b.title)
    return 1
    if (a.title < b.title)
    return -1
  ...
  console.log ('Orden alfabetico', aux)
}
