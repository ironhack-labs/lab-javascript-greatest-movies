...
function scoresAverage(movies) {
  ...
  const totalScore = movies.reduce((score, movie) => score += !movie.score ? 0 : movie.score,0)
...
function orderAlphabetically(movies) {
  ...
  return orderedArray.sort((a,b) => a.title.localeCompare(b.title)).slice(0,20).map(movie => movie.title);
}
...
function turnHoursToMinutes(movies) {
  ...
  let newArr = movies.map(movie => {
    ...
    if(duration.length < 2){
      hours = 0;
      min = parseFloat(duration)
    }else{
      hours = parseFloat(duration[0]) * 60;
      mins = duration[1] ? parseFloat(duration[1]): 0;
    }
    duration = hours + mins;
