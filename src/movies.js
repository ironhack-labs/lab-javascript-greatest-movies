...
function scoresAverage(moviesArray) {
  ...
  const allScores = moviesArray.reduce(function (acc, movie) {
    let score = movie.score;
    if (score === undefined || score === '') {
      score = 0;
    }
    return acc + score }, 0);
  if ((allScores / moviesArray.length) === Math.trunc(allScores / moviesArray.length)) {
...
function dramaMoviesScore(moviesArray) {
  const allDramas = moviesArray.filter((movie) => {
    return movie.genre.indexOf('Drama') >= 0
  });
  if (allDramas.length === 0) {
    return 0
  }
  let allDramasScores = allDramas.reduce(function (acc, movie) {
    return acc + movie.score }, 0);
    if ((allDramasScores / allDramas.length) === Math.trunc(allDramasScores / allDramas.length)) {
    return (allDramasScores / allDramas.length);
  } else {
...
function orderByYear(moviesArray) {
  let compareYear = moviesArray
  compareYear.sort(function (a, b) {
    if (a.year === b.year) {
      if (a.title < b.title ) {
        return -1
...
function orderAlphabetically(moviesArray) {
  let moviesAlphabetically = []
  moviesArray.forEach(element => {
    moviesAlphabetically.push(element.title)
  });
  moviesAlphabetically.sort(function (a, b) {
    if (a > b) {
      return 1;
     } else if (a < b) {
       return -1; 
...
function turnHoursToMinutes (moviesArray) {
  ...
  moviesArray.forEach(element => {
    ...
    if (element.duration[0] === '0') {
      duration = Number(element.duration[3] + element.duration[4])
    } else if(element.duration[4] >= 0) {
      let hour = Number(element.duration[0] * 60)
      let minuteTens = Number(element.duration[3] * 10)
      let minuteUnits = Number(element.duration[4])
      duration = hour + minuteTens + minuteUnits
    } else if (element.duration.length === 2) {
      duration = Number(element.duration[0] * 60)
    } else {
      duration = Number(element.duration[0] * 60) + Number(element.duration[3])
    }
    ...
    moviesDuration.push(newElement)
  });
