...
// My pull link is: https://github.com/ironhack-labs/lab-javascript-greatest-movies/pull/3667
...
function dramaMoviesScore(moviesArray) {
    ...
    const roundedResult = parseFloat((averageDramaScore).toFixed(2))
...
function orderByYear(moviesArray) {
    ...
    const moviesByYear = copyMoviesArray.sort((a,b)=>{
        if (a.year===b.year){
...
function orderAlphabetically(moviesArray) {
    const copyMoviesArray =[...moviesArray]
    const moviesByName = copyMoviesArray.sort((a,b)=>
    a.title.localeCompare(b.title))
    const twentyMovies = moviesByName.slice(0,20)
    const onlyTitles = twentyMovies.map(element=>element.title)
