...
function howManyMovies(movies) {
    const dramaS = movies.filter(movie => movie.director === "Steven Spielberg" && movie.genre.includes('Drama'))
    return dramaS.length
    }

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
/*INTENTO SIN PARSEFLOAT
function scoresAverage(movies) {
  const prom = movies.reduce(function (acumulador, actual) {
  ...
  let finalScore = Math.round(prom / movies.length).toFixed(2)
  return Math.round(finalScore).toFixed(2)
}
*/
//INTENTO CON PARSEFLOAT (no es dependiente de ningún objeto)
function scoresAverage(movies) { 
  let prom = movies.reduce((acumulado, actual) => { 
     return  acumulado + actual.score 
  }, 0) 
  if(movies.length === 0){
   return 0;
 }          //no se ejecuta con Math.round al inicio ¿Por?
  let result =  (prom/movies.length).toFixed(2); 
...
function dramaMoviesScore(movies) {
  ...
  let allDramas =
    movies.filter((upd) => {
      if (movies.genre.includes('Drama')) {return upd;}
    })
  if (!allDramas.length) return 0;
  let finalScore = allDramas.reduce((acc, act) => {
    if (movies.score && movies.genre.includes('Drama')) {
      return acc + act.score;
    } else { return acc;
    }
  }, 0);
  return Math.round(finalScore / allDramas.length)
}*/
//INTENTO CON PARSEFLOAT
function dramaMoviesScore(movies) { 
  let dramaMovie = movies.filter((da) => da.genre.includes('Drama')); 
  let calculateAll = dramaMovie.reduce((acum, actu) => {
    return acum + actu.score;
  },0)           //no se ejecuta con Math.round al inicio ¿Por?  
  let final = (calculateAll/dramaMovie.length).toFixed(2); 
...
// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
  function orderByYear(movies) {
    const newMovieArray = movies.year(movie => movie.year(movies));
