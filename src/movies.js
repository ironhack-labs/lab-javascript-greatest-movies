...
const howManyMovies = () => {
const manyMovies = movies.filter((elem) => {
...
const ratesAverage = () => {
const ratesAvg = movies.reduce((a,b) => {
...
/*const howManyDramaMovies = () => {
  const manyDramaMovies = movies.filter((elem) => {
  return ( elem.genre.includes("Drama"))
})
return manyDramaMovies}*/
...
const dramaMoviesRate = () => {
  ...
  const dMoviesRate = manyDramaMovies.reduce((a,b) => {
  const totalDramaPeliculas = manyDramaMovies.length
  const totalDramaRate = a + b.rate/totalDramaPeliculas
  return totalDramaRate
},0)
return dMoviesRate.toFixed(2) }
...
const moviesSortByTitle = movies.slice(0)
const orderAlphabetically = () => {
  
  movieByTitle =[]  
  const byTitle = moviesSortByTitle.sort((a,b) =>  {
    moviesSorted = a.title.localeCompare(b.title) ;
    return moviesSorted
  }) 
    if(byTitle.length > 20) {  
      for (i = 0 ; i < 20; i++) {
        movieByTitle.push(byTitle[i].title)       
  }
  } else {for (i = 0 ; i < byTitle.length ;i++) {
        movieByTitle.push(byTitle.title)}
  } return movieByTitle }


  
// BONUS - Iteration 7: Time Format - Turn duration of the movies from hours to minutes
const moviesNewDuration = movies.slice(0)

const turnHoursToMinutes = () => {
  const hoursToMinutes = movies.map((time) => {
    const hour = time.duration.substring(time.duration.indexOf("h")-1,1)*60
    
    const  minutes = Number(time.duration.substring(time.duration.indexOf("i")-1,2))
    const newDuration = hour + minutes

    return newDuration
  })
  
   for (j = 0; j < hoursToMinutes; j++){
    for (let i =0; i <  moviesNewDuration; i++){
    moviesNewDuration[i].duration = hoursToMinutes[j]
    
  }}


  return moviesNewDuration
}
