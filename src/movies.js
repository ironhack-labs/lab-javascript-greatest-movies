...
function bestYearAvg(moviesArray) {
  ...
  years.forEach((year) => {
    const moviesPerYear = moviesArray.filter((movie) => movie.year === year);
    const avg = scoresAverage(moviesPerYear);
    avgPerYear.push({
      year,
      avg,
    });
  });
  const bestYear = avgPerYear.sort((a, b) => a.avg - b.avg).reverse()[0];
