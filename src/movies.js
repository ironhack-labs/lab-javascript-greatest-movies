...
function getAllDirectorsUniq(movies) {
  let arr = movies.map((mov) => mov.director);
  return [...new Set(arr)];
}

getAllDirectorsUniq(movies);
...
// 1.functions order is crutial, sort first, then slice
...
function turnHoursToMinutes(arr) {
  let movDurArr = [...arr].map((mov) => mov.duration);
  let regex = /(\d+)h\s(\d+)min/;
  let regMovDurArr = movDurArr.map((item) => item.match(regex));
  //console.log(regMovDurArr);

  return regMovDurArr.map(function (el) {
    if (typeof el !== 'undefined') {
      return Number(el[1] * 60) + Number(el[2]);
    } else return el === 0;
  });
}

//turnHoursToMinutes(movies7);
console.log(turnHoursToMinutes(movies7));
...
function bestYearAvg(arr) {
  let movYearArr = arr.map((mov) => mov.year);
  console.log(movYearArr);

  //return `The best year was ${YEAR} with an average score of ${RATE}.`;
}

bestYearAvg(movies8);
//console.log(bestYearAvg(movies8));
