...
function orderByYear(movies) {
  ...
  newArray.sort(function(objA, objB) {
...
function howManyMovies(movies) {
  const newArray = movies.filter(function(genereObj, i, movies) {
    if (genereObj.genre == "Drama") {
      return true;
    }
  });
...
function ratesAverage(movies) {
  const eAverage = movies.reduce(function(acc, averageObj, i, movies) {
    let updatedMovies = acc + averageObj.rate;

    return updatedMovies;
  }, 0);

  var num = eAverage / movies.length;
  var n = num.toFixed(2);
  console.log(n);
}
ratesAverage(movies);
...
function dramaMoviesRate(movies) {
  ...
  const eAverage = newArray.reduce(function(acc, averageObj, i, newArray) {
    let updatedMovies = acc + averageObj.rate;

    return updatedMovies;
  }, 0);

  var num = eAverage / newArray.length;
  var n = num.toFixed(2);
  console.log(n);
}
