...
function getAllDirectors() {
    let allMovies = movies.map(eachMovie => {
...
function howManyMovies() {
    let stevenMovies = movies.filter(eachMovie => {
        return eachMovie.director.toLowerCase().includes('steven spielberg')
    })
    return stevenMovies
}
...
function scoresAverage() {
    let score = movies.reduce((acc, eachMovie) => {
        return (acc + eachMovie.score)
    }, 0)
    return (score / movies.length).toFixed(2)
}

//console.log(scoresAverage())
// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore() {
    let drama = movies.filter(eachMovie => {
        return eachMovie.genre.includes('Drama')
    })
    let average = drama.reduce((acc, eachDrama) => {
...
function orderAlphabetically() {
    ...
    let slicetitle = titleSort.slice(0, 20)
    return slicetitle
...
// let turnHours = JSON.parse(JSON.stringify(movies))
// turnHours.duration = "20"
// console.log(turnHours)
// let change = movies.parseInt(duration)
// console.log(change)
