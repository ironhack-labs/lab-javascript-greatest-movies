...
let cleanDirectorsFunction = arr => {
    let cleanDirectorsArray = []
    arr.forEach(element => {
        if ( cleanDirectorsArray.indexOf(element) === -1) {
            cleanDirectorsArray.push(element)
        }
    })
    return cleanDirectorsArray
}

console.log(cleanDirectorsFunction(getAllDirectors(movies)))
