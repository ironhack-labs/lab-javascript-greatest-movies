...
function scoresAverage(movies) {
  if (movies.length === 0) {
    ...
    scoresArray = movies.map(function (film) {
      return film.score;
    });
  }
  const totalScores = scoresArray.reduce(function (acc, curr) {
    return acc + curr;
  }, 0);
  const averageScore = Math.round((totalScores / movies.length) * 100) / 100;
  return averageScore;
}
