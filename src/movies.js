...
function ratesAverage(movies) {
  ...
  //console.log("test: ", average.toFixed(2))
...
function orderByYear(movies) {
  const sortByYear = movies.concat().sort(function (movie_a, movie_b) {

    let yearSort = movie_a.year - movie_b.year;
    if (yearSort === 0) {
      return movie_a.title.localeCompare(movie_b.title);
    } else {
      return yearSort;
    }
  });
...
function orderAlphabetically(movies) {
  const sortTitle = movies.sort(function (a, b) {
   let titleSort = a.title - b.title;
   
   return a.title.localcompare(b.title);
  });
  console.log(sortTitle);
  return sortTitle.slice(19); 
