...
function getAllDirectors(moviesArray) {
    /* return moviesArray
        .map(function(directorElements) {
            return directorElements.director
        })
} */    

        return moviesArray.map(function(directorElements) { /* --> note to me: hier map und nicht filter, weil wir ja ein neuen Array haben möchten, in dem nur die directors erscheinen. Filter würde hier nur was bringen
        wenn wir nach wie vor alle Infos der Elemente im Array haben möchten aber z.B. nur die, die von Steven Spielberg gedreht wurden (siehe Aufgabe unten*/
            return directorElements.director 
})
}

console.log(getAllDirectors(movies)) /* --> müsste doch eg funktionieren bzw. wie verlinke ich das??  */
console.log("test")

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
    function howManyMovies(moviesArray) {
        return moviesArray
             .filter(function(moviesWithSteven) {
                if (moviesWithSteven.director === "Steven Spielberg") {
                    return moviesWithSteven.director
                    }
                }) 
                .filter(function(dramasWithSteven) {
                return dramasWithSteven.genre.includes("Drama")
                })
                .length
            }  
...
function dramaMoviesScore(moviesArray) {
    return scoresAverage( /* -->  das funktioniert, weil ich fct calle und mit dem filter (der mir ja
        ein Array ausgibt, diesen Array (=Argument) als Parameter einsetze.*/
        /* moviesArray.filter(function(movie) {
            if (movie.genre.includes("Drama")) {
                return movie
            }
        }) */
        moviesArray.filter (movie => movie.genre.includes("Drama")) /* --> das ist mit arrow fct */
    )
}
...
function orderByYear(moviesArray) {
    ...
    return moviesCopy.sort(function(a, b) {
