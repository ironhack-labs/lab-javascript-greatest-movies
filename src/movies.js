...
function scoresAverage(moviesArray) {
    if (moviesArray.length === 0) {
      ...
      const sumOfScores = moviesArray.reduce((accumulator, movies) => {
...
function dramaMoviesScore(moviesArray) {
    const theDramaMovies = moviesArray.filter((movies) => {
    movies.genre.includes('Drama');
  });
  averageScore = scoresAverage(theDramaMovies);
  return averageScore;
}
...
function orderByYear(moviesArray) {
    ...
    moviesClone.sort((first, second) => {
        if (first.year > second.year) {
            ...
            return first.title.localCompare(second.title);
        }
    });
    return moviesClone
}
