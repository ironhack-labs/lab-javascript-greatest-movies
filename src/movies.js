...
function getAllDirectors(moviesArray) {
    let directorsName = [];
    directorsName = moviesArray.map((i) => {
        return i.director;
    });
...
function orderByYear(moviesArray) {
    let sortedMovies = [...moviesArray]//a copy
...
function orderAlphabetically(moviesArray) {
    const orderMovie = moviesArray.map(function(movie){
        return movie.title;
    })
    const lastOrder = orderMovie.sort().splice(0, 20);
    return lastOrder;
}
