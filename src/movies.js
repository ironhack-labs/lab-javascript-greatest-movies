...
function howManyMovies(data) {
  ...
  const dramaMovies = numberMovies.filter((elem) => {
    const genre = elem.genre.includes('Drama') ? true : false;
    return genre ? elem : null;
  });
...
function scoresAverage(data) {
  if (data.length !== 0) {
    ...
    // Argregue el parseFloat por que me regresaba una string y el test pedía que fuera un numero
    const roundScore = parseFloat(scorePromedio.toFixed(2));
    return roundScore;
  }
  return 0;
}
