...
function getAllDirectors(phMovies) {
  const directors = phMovies.map(function (movie) {
...
function howManyMovies(arraySpiel) {
  const spielberg = arraySpiel.filter(function (movie) {
    return (
...
//ES6 TEST
// function howManyMovies(phMovies) {
//   const spielberg = phMovies.filter(
...
function ratesAverage(phMovies) {
  const average = phMovies.reduce(function (acc, movie) {
    return acc + movie.rate;
  }, 0);
  return Math.round((average / phMovies.length) * 100) / 100;
}
