...
function getAllDirectors(movies) {
  ...
  let mappedArray = movies.map(movie => {return movie.director})
...
function howManyMovies(movies) {
  let moviesSpielberg = movies.filter(movie => movie.genre.indexOf('Drama')>=0 && movie.director === "Steven Spielberg");
  let result = moviesSpielberg.length;
  return result
}
...
function scoresAverage(movies) {
  ...
  let averageScore = movies.reduce((acc,movie) => {
    if (movie.score){
      return acc + movie.score  
    }
    return acc;
  },0);
  let result = (averageScore/size).toFixed(2);  
  return parseFloat(result) 
}
// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(movies) {
  let dramaGenre = movies.filter (movie =>movie.genre.indexOf('Drama')>=0)
  let dramaSize = dramaGenre.length
  let dramaAverage = dramaGenre.reduce((acc,movie) => acc + dramaGenre.score,0)
  let result = (dramaAverage/dramaSize).toFixed(2);
  return parseFloat(result); 
}
