...
function getAllDirectors(moviesArray) {
  const arrDirectors = moviesArray.map((movie) => movie.director);
  const singleDirectors = [];

  for (let i = 0; i < moviesArray.length; i++)
    if (singleDirectors.includes(arrDirectors[i])) {
      continue;
    } else {
      singleDirectors.push(arrDirectors[i]);
    }
...
function howManyMovies(moviesArray) {
  const howManyMovies = moviesArray.filter((movie) => movie.director === 'Steven Spielberg' && movie.genre.includes('Drama'));
...
function scoresAverage(moviesArray) {
  ...
  const arrayOfScores = moviesArray.map((movie) => movie.score);

  const totalMovieScore = arrayOfScores.reduce((acc, curr) => acc + curr);
  return parseFloat((totalMovieScore / moviesArray.length).toFixed(2));
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(moviesArray) {
  const dramaMovies = moviesArray.filter((movie) => movie.genre.includes('Drama') && movie.genre.length === 1);

  if (dramaMovies.length === 0) {
    return 0;
  }

  const arrayOfScores = dramaMovies.map((movie) => movie.score);
  const totalMovieScore = arrayOfScores.reduce((acc, curr) => acc + curr);

  return parseFloat((totalMovieScore / dramaMovies.length).toFixed(2));
}
