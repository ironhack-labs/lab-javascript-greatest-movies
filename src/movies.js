...
function scoresAverage(moviesArray) {
    ...
    const sumOfScores = moviesArray.reduce((acc, curr)=>{
        if (curr.score){
    ...
    const pizza = (sumOfScores / moviesArray.length).toFixed(2);
    return +pizza;} 
...
function orderByYear(moviesArray) {
    const newArray = moviesArray.map((films) => {
        return films;
    })
    const sortedByYear = newArray.sort((a, b) => {
        if (a.year > b.year) {
          return 1
        }
