...
function getAllDirectors(movies) {
  const mapped = movies.map( function (movies) { 
...
function scoresAverage(movies) {
  ...
  const filtered = movies.filter(movies => Boolean(movies.score) === true);
  const scores = filtered.map( function(filtered) {
     return filtered.score; 
  });
  const sumScores = scores.reduce((total, next) => total + next);
...
function dramaMoviesScore(movies) {
  ...
  const filtered = movies.filter(movies => Boolean(movies.score) === true && movies.genre.includes('Drama'));
...
function orderAlphabetically(movies) {
  const filtered = movies.filter(movies => Boolean(movies.title) === true);
