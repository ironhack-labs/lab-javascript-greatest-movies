...
const scoresAverage = (movies) => isEmpty(movies) ? 0 : decimalOperation(movies.map((obj) => obj.score).filter((score) => typeof score === 'number').reduce((acc, cur) => acc + (cur / movies.length), 0))
