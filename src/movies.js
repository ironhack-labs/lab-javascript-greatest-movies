...
function orderByYear(moviesArray) {
  let moviesArrayClone = moviesArray.filter((movie) => 1);
  let moviesByYear = moviesArrayClone.sort((a, b) => a.year === b.year ? a.title.localeCompare(b.title) : a.year - b.year);
  return moviesByYear;
}
...
function turnHoursToMinutes(moviesArray) {
  ...
  let result = moviesArray.map((movie) => {
    ...
    if (movie.duration.includes("h") && movie.duration.includes("min")) {
      ...
      movieDuration = (parseInt(durationArr[0]) * 60) + parseInt(durationArr[1]);
    } else if (movie.duration.includes("h") && !movie.duration.includes("min")) {
      movieDuration = parseInt(movie.duration) * 60;
    } else {
