...
lel = (movieArray(movies).reduce((curSum, curReview) => curSum + curReview))/ movies.length
let averageToDec = Number(lel.toFixed(2))
...
//    return a + Number(b.rate) / movies.length
//  })
...
function howManyMovies(movies){
 let spielbergMovies = movies.filter( carrots => 
    carrots.director === "Steven Spielberg")
    .filter( carrots => 
      carrots.genre === "Drama")
    .reduce((sum,elem) => {
...
function orderAlphabetically(movies){
  ...
  sortedArr= [...movies].sort((a,b) =>
    ...
    for(let i=0 ; i <= 19 ; i++){
      sortedFirst20.push(sortedArr[i].title); 
    };

  return sortedFirst20

}

//use sort -> title and then loop for first 20


// Best yearly rate average


//use reduce for average -> return only one year which will be the maximum average rate
