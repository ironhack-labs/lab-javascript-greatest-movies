...
function turnHoursToMinutes(movies){
      //var moviesResult = Object.assign(movies)
      moviesResult = movies.map(function(movie){
      var count = movie.duration.replace("h", "").replace("min","").split(" ");
      var totalMinutes = parseInt(count[0])*60 + parseInt(count[1]);
      movie.duration = totalMinutes;
      return movie;
      });
    return moviesResult;
}
var onlyNumbers = turnHoursToMinutes(movies);
console.log(movies);
...
function dramaMoviesRate(movies){
  ...
  var rateDrama = obj.reduce(function(sum, movie){
    sum += movie.rate / obj.length;
    return sum; 
  }, 0)
  return rateDrama.toFixed(2); 
}
