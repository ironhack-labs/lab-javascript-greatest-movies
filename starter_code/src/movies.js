...
function dramaMoviesRate(movies) {
  ...
  if (dramas.length === 0) {
    ...
    var sum = dramas.reduce(function(acc, drama) {
      if (drama.rate === "") {
        return acc;
      } else {
...
// Sort the movies based on their duration
...
// How many dramas Steven Spielberg has
...
// 20 movies order alphabetically
...
function bestYearAvg(movies) {
  var avg = []; // array of average rate of the year
  ...
  if (movies.length === 0) {
    ...
    // 1. sort the movies array by year
    movies.sort(function(m1, m2) {
    ...
    for (var i = 0; i < movies.length; i++) {
      if (i + 1 !== movies.length && movies[i].year === movies[i + 1].year) {
        ...
        //reset counter and sumRate initial value
        counter = 1;
    ...
    // sort: highest to lowest rating, if same rating, oldest year comes first
    // after sorting, the first element of the avg array is the best of the year
