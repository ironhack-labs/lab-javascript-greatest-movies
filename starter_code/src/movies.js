...
const turnHoursToMinutes = arr => {
  ...
  return movies.map(movie => {
    if (!movie.duration.includes('h')) {
      movie.duration = parseInt(movie.duration.substr(0, 2)); 
      return movie;
    }

    let supArr = movie.duration.split(' ').join('').split('h');
    supArr[0] = parseInt(supArr[0]) * 60;
    if(!movie.duration.includes('min')) {
      movie.duration = parseInt(supArr[0]);
      return movie;
    }

    supArr[1] = parseInt(supArr[1].substr(0, 2));
    movie.duration = parseInt(supArr.reduce((acc, time) => acc + time));
    return movie;
  });
};
...
const ratesAverage = movies => {
  return parseFloat(movies.map(movie => movie.rate === '' ? 0 : parseFloat(movie.rate)).reduce((acc, rate) => acc + rate / movies.length, 0).toFixed(2));
}
...
const orderByDuration = movies => {
  ...
  let orderedMovies = JSON.parse(JSON.stringify(movies))
...
const howManyMovies = movies => {
  ...
  let SPmovies = movies.filter(movie => movie.director === 'Steven Spielberg' && movie.genre.includes('Drama'));
  if (SPmovies.length === 0)
    return 'Steven Spielberg directed 0 drama movies!';
  
  if (SPmovies.length === 1) 
    return 'Steven Spielberg directed 1 drama movies!';

  if (SPmovies.length === movies.length)
...
const orderAlphabetically = movies => {
  ...
  let orderedArray = JSON.parse(JSON.stringify(movies));
  orderedArray.sort((a, b) => a.title.localeCompare(b.title));

  return orderedArray.slice(0, 20).map(movie => movie.title);
};
...
const bestYearAvg = movies => {
  ...
  movies.forEach(movie => {
    if(!yearRates.includes(movie.year))
      yearRates[yearRates.length] = movie.year;
  });

  yearRates = yearRates.map(y => {
    return {year: y, avgRate: 0}
  });

  yearRates.forEach((obj, index) => {
    let moviesInThatYear = movies.filter(movie => movie.year === obj.year);
    yearRates[index].avgRate = ratesAverage(moviesInThatYear);
  });
  ...
  yearRates.forEach((obj, index) => {
    if (obj.avgRate > max) {
      ...
      if (parseInt(obj.year) < parseInt(yearRates[maxIndex].year)) {
        max = obj.avgRate;
        maxIndex = index;
      }
    }
  ...
  return `The best year was ${yearRates[maxIndex].year} with an average rate of ${yearRates[maxIndex].avgRate}`;
}
