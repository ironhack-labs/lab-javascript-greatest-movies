...
// These below extract the numbers only of duration key, and adds them together. 

var durationsArray1 = movies.map(function(movie){
  return movie.duration.slice(0,1)*60} )
  
var durationsArray2 = movies.map(function(movie){
  return movie.duration.slice(3,5)*1} )
  
var durationsArrayFinal = durationsArray1.map(function(time){int = 0;
int = int+1
  return time + durationsArray2[int+1]
  // how do I get the int to loop????
  
})
  

console.log(durationsArray1)
console.log(durationsArray2)
console.log(durationsArrayFinal)

// This keeps all the following information (I left out Duration)

movieArrayWithoutDuration = movies.map(movie => ({ 
  title: movie.title,
    year: movie.director,
    genre:movie.genre,
    rate:movie.rate,
...
console.log(movieArrayWithoutDuration);

// Let's add Durations to Array I just created
function arrayToList (array){
    var dataStructure = null;
    for (var i = 0;i < array.length; i++){
    dataStructure= {duration: array[i]};
    }
    return dataStructure;
}
arrayToList(durationArrayFinal)





// How do I change all of the durations to the number only?
...
// Get the average of all rates with 2 decimals //Because I cannot get the full array with only numbers in duration, I'm going to reference the original movies Array in this section
