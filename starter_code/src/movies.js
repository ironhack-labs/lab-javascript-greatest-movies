...
function turnHoursToMinutes(arr) {
  return arr.map(function(val) {
      if (val.duration.search(/[a-zA-Z]/gi) > -1) {val.duration = val.duration.replace(/[a-zA-Z]/gi, '')};
      val.duration = val.duration.split(' ');      
      if (val.duration.length > 1) {
        val.duration[0] = parseInt(val.duration[0] * 60);
      }
      val.duration = val.duration.map(Number).reduce((acc,cur) => acc+cur);
      return val
    }
  )
}
...
function ratesAverage(arr) {  
  var output = arr.reduce((acc, cur) => acc + parseFloat(cur.rate),0) / arr.length;
...
function orderByDuration(arr) {
  return turnHoursToMinutes(arr).sort(function(a,b) {
...
function howManyMovies(arr) {
  var output = arr.filter((val) => val.genre.filter((val) => val.includes("Drama")));
  //return output.filter((val) => val.director === 'Steven Spielberg') //insutrction are not clear;
  //returning a number?
  return output.filter((val) => val.director === 'Steven Spielberg').length
}
...
function orderAlphabetically(arr) {
  var output = arr.slice(0, Math.min(19, arr.length-1)).map((val) => val.title);
...
function bestYearlyAverage(arr) {
  ...
  for(var i=0; i<years.length; i++){
    yearsAvg.push([years[i], ratesAverage(movies.filter((val) => val.year === years[i]))])
  }  
  yearsAvg = yearsAvg.sort((a,b) => b[1] - a[1]);
  if (yearsAvg[0][1] !== yearsAvg[1][1]) {
    return yearsAvg[0]
  }
  else {
    for(var i=0;i<yearsAvg.length;i++) {
      if (yearsAvg[0][1] !== yearsAvg[i][1]) {
        return yearsAvg.slice(0, i-1)
      }
    }
  }  
}
