...
function turnHoursToMinutes(array) {
  ...
  let duration = newArr.map(movie => {
    let hour = movie.duration.split('');
    let hourConverted = 0; 
    let minutes = 0;
    if(hour.length === 2) {
      hourConverted = parseInt(hour[0] *60);
      movie.duration = hourConverted
        } else if(hour.length === 5) {
          minutes = (parseInt(hour[0]*10) + parseInt(hour[1]));
          movie.duration = minutes;
        } else if(hour.length === 7) {
          hourConverted = parseInt(hour[0] *60);
          minutes = (parseInt(hour[3]))
          movie.duration = hourConverted + minutes;
...
function ratesAverage(arr) {
  let sumRate = arr.reduce((acc, item) => acc + parseFloat(item.rate), 0);
  let average = parseFloat((sumRate / arr.length).toFixed(2));
  return average;
}

// // Get the average of Drama Movies
function dramaMoviesRate(arr) {
  let dramaMovies = arr.filter(item => item.genre.includes('Drama'));
  console.log(dramaMovies)
  if (dramaMovies.length === 0)
  return;
  let sumRate = dramaMovies.reduce((acc, item) => {
  if (item.rate == '')
    return acc;
...
const orderByDuration = (arr) => {
  let orderedByDuration = arr.sort((a, b) => {
...
const howManyMovies = (arr) => {
  ...
  let dramaMovies = arr.filter(item => item.genre.includes('Drama'));
  let spielbergFilms = dramaMovies.filter(item => item.director.includes('Steven Spielberg'));
