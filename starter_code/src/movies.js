...
function ratesAverage(movies) {
    totalMovies = movies.reduce(function(acc, raid){
        return acc + parseFloat(raid.rate)
    }, 0)
    var avgMovie = totalMovies / movies.length;
    return avgMovie;
}

// Get the average of Drama Movies

let arr = movies.map(function(x){
...
function dramaMoviesRate(arr) {
    let dramaM = arr.reduce(function(x, y){
        return x + parseFloat(y.rate)
    }, 0)
    let dramaMovie = dramaM / arr.length;
    return dramaMovie;
}
