...
function turnHoursToMinutes(arr) {
    const newObj = JSON.parse(JSON.stringify(arr));
    return newObj.map(function(e){ 
...
function ratesAverage(arr) {
  const newObj = JSON.parse(JSON.stringify(arr));
  newObj.map(function(e){
    e.rate = parseFloat(e.rate);
  });
  let newAvg = newObj.reduce(function(acc, e){
    return acc + e.rate;    
  }, 0); 
  let result = newAvg/newObj.length;
...
function dramaMoviesRate(arr) {
  ...
  for(let i=0;i<newObj.length;i++) {
    for(let j=0; j < newObj[i].genre.length; j++) {
      if ( newObj[i].genre[j] === 'Drama'){
        listDrama.push(newObj[i]);
      }
    }
  }
  let newAvg = listDrama.reduce(function(acc, e){
    return acc + e.rate;   
  }, 0); 
  let result = newAvg/newObj.length;
  result = result.toFixed(2);
...
/*function orderByDuration(arr) {
  ...
  let orderList = [];
  orderList = turnHoursToMinutes(newObj);
...
function orderByDuration(arr) {
  ...
  let minutesList = [];
  let orderList = [];
  minutesList = turnHoursToMinutes(newObj);


  for(let i = 0; i < minutesList.length; i++){
    orderList.push(minutesList[i].duration);
  }
  orderList.sort(function(a, b) {
    return a - b;
  });
...
function howManyMovies(arr) {
  ...
  let directorSteve = [];
  directorSteve = listDrama.filter(function(e) {
      return e.director === 'Steven Spielberg';
  })
  if (directorSteve === []){
    return undefined;
  }else{
  return `Steven Spielberg directed ${directorSteve.length} drama movies!`;
  }
}
...
function orderAlphabetically(arr) {
  ...
  for(let i = 0; i < newObj.length; i++){
    orderList.push(newObj[i].title);
  }
  function orderTitle(a,b) {
    if (a < b)
      return -1;
    if (a > b)
      return 1;
  ...
  titleList20 = orderList.sort(orderTitle);
  if(titleList20.length < 20){
    return titleList20;
  }else{
    for(let i=0; i < 20; i++){
      titleListTotal.push(titleList20[i]);
    }
  }
  return titleList20;
}
