...
function turnHoursToMinutes(array){
    let minutesArray=array.map(function(movie){  
        let hIndex=0;
        let spaceIndex=0;
        let minIndex=0;
        let min=0;
        ...
        if(movie.duration.indexOf('h')>-1){
            hIndex=movie.duration.indexOf('h');
            spaceIndex=movie.duration.indexOf(' ');
            horas=parseInt(movie.duration.substring(0,hIndex));
        }
            
        if(movie.duration.indexOf("min")>-1){
            minIndex=movie.duration.indexOf("min");
            min=parseInt(movie.duration.substring(spaceIndex,minIndex));
        }             
        //movie.duration=horas*60+min; 
        return{

            title: movie.title,
            year: movie.year,
            director: movie.director,
            duration: horas*60+min,
            genre: movie.genre,
            rate: movie.rate
        };
    }); 
...
function ratesAverage(array){
    ...
    let totalSum=array.reduce(function(rateSum,movie){   
        ...
        if(movie.rate==undefined){
            ...
            return rateSum+=parseFloat(movie.rate);
        }
            
    },0);
    //console.log(totalSum);
    return parseFloat((totalSum/totalMovies).toFixed(2));
}
//console.log(ratesAverage(movies));
//console.log(parseFloat(8));
// Get the average of Drama Movies
function dramaMoviesRate(array){
    let dramaFiltered=array.filter(function(movie){
        return movie.genre.indexOf("Drama")!=-1;
    });
    if(dramaFiltered.length==0){
        return undefined;
    }else{
        return ratesAverage(dramaFiltered);
    }
}
...
function howManyMovies(array){
  ...
  let stevenDrama=dramaFiltered.filter(function(movie){
...
function orderAlphabetically(array){
  ...
  //var spliced=array.splice(0,20);
  let result=[];
  for(let i=0;i<sorted.length;i++){
    if(i<20){
      result.push(sorted[i].title+"");
    }
...
function bestYearAvg(array){
  ...
  for(let i=0;i<years.length;i++){
    if(yearsNoDupl.indexOf(years[i])==-1){
      yearsNoDupl.push(years[i]);
    }
  }
  yearsNoDupl.sort(function(a,b){
    
    return a-b;
  });
  let proms=[];
  for(let i=0;i<yearsNoDupl.length;i++){
    let filteredYear=array.filter(function(movie){
      return movie.year==yearsNoDupl[i];
    });
    proms[i]=ratesAverage(filteredYear);
  }
  var greatestIndex=proms.indexOf(Math.max(...proms));
  console.log("greatest: "+greatestIndex);
console.log(yearsNoDupl);
console.log(proms);
console.log(yearsNoDupl.length);
console.log(proms.length);
return "The best year was "+yearsNoDupl[greatestIndex]+" with an average rate of "+proms[greatestIndex];
