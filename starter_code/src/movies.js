...
function turnHoursToMinutes(arr) {
	...
	newArr.forEach(function(movie) {
		...
		var minsString = minFirstDig + minSecondDig; //works
		var minsToNumbers = Number(minsString); //works
		var minutes = hours + minsToNumbers; //works
...
function ratesAverage(arr) {
	//iterates through the arr, enters each object and collects movie rates into a new array
	var rates = arr.map(function(movie) {
		return Number(movie.rate);
	});

	//iterates thrhough new array of movie rates and totals them
	var totalRates = rates.reduce(function(acc, curr) {
		return acc + curr;
	}, 0);

	return Math.round(totalRates / arr.length * 100) / 100;
}

ratesAverage(movies);

// Get the average of Drama Movies
function dramaMoviesRates(arr) {
	//filters drama movies into a new array
	...
	//creates array with rates of drama movies
	var dramaRates = dramaMovies.map(function(movie) {
		return Number(movie.rate);
	});

	//totals the drama movie rates
	var totalDramaRates = dramaRates.reduce(function(acc, curr) {
		return acc + curr;
	}, 0);
...
dramaMoviesRates(movies);

// Order by time duration, in growing order

function orderByDuration(arr) {
	//access movies by duration
