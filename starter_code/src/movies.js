...
function turnHoursToMinutes(arrayOfMovies) {
    var arrayMinutesMovies = arrayOfMovies.map(function(element) {
        var movie = Object.assign({}, element)
        if (movie.duration.includes("h") && movie.duration.includes("min")) {
            movie.duration = movie.duration[0] * 60 + parseInt(movie.duration.substr(3, 2))
        } else if (movie.duration.includes("h")) {
            movie.duration = movie.duration[0] * 60
        } else {
            movie.duration = parseInt(movie.duration.substr(0, 2))
        }
        return movie
    })
    return arrayMinutesMovies
}
