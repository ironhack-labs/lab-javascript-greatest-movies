...
const turnHoursToMinutes = (movies) => {
  let newArray = JSON.parse(JSON.stringify(movies));
    ...
    let hours = movie.duration[1] === 'h' ? parseInt(movie.duration[0]) : 0;
    let minutes = movie.duration.endsWith('n') ? parseInt(movie.duration.slice(-5, -3)) : 0;
...
const ratesAverage = (movies) => {
  ...
  return movies.reduce((acc, movie) => {
    avg += parseFloat(movie.rate) / movies.length;
...
const dramaMoviesRate = (movies) => {
  const imdbTopDrama = movies.filter(movie => movie.genre.includes('Drama'));
  if (imdbTopDrama.length < 1) {
    return undefined;
  } else {
    return imdbTopDrama.reduce((acc, movie) => {
      movie.rate !== '' ? 
      acc += parseFloat(movie.rate) / movies.filter(movie => movie.genre.includes('Drama')).length :
      acc += 0;
...
const howManyMovies = (movies) => {
  if (movies.length < 1) {
    ...
    let stevenDramas = movies.filter(movie => movie.director === 'Steven Spielberg' && movie.genre.includes('Drama'));
    return `Steven Spielberg directed ${stevenDramas.length} drama movies!`;
  }
}
