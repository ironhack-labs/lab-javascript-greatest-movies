...
function ratesAverage (array){
  //This function sums the rates of all movies
  totalRates = array.reduce(function (acc, aFilm){
    return acc + parseFloat(aFilm.rate);
  }, 0);

  // an average is equal to the total amount of an array of numbers divided by the number of elements
  var averageRate = totalRates/array.length;

  //we should return the average rounded to two decimals, unless the average is an integer
  if (Number.isInteger(averageRate)=== true){
    return averageRate;
  } else {
...
// ITERATION 2 : Get the average of Drama Movies

//We create a new array composed of only drama movies
var dramaMovies = movies.filter(function(aMovie){
  return aMovie.genre.includes('Drama')
});

//we create a new array composed only of drama movies that have a rate property
// var ratedDramaMovies = dramaMovies.filter(function(aDramaMovie){
//   return aDramaMovie.rate;
// });

var ratedDramaMovies = [];
var nnn=0

dramaMovies.forEach(function (dramaMovie){
  if (!dramaMovie.rate){
    nnn++
    ratedDramaMovies.push(dramaMovie);
  }
})

console.log(ratedDramaMovies);



//we sum up the total amount of all drama movies rates
var dramaRates = 0;
ratedDramaMovies.forEach(function(aMovie){
  dramaRates += aMovie.rate;
  return parseFloat(dramaRates);
})


function dramaMoviesRate (myArray){
  return ratesAverage(myArray);
  };

console.log(dramaMoviesRate(ratedDramaMovies));

// var sumDMRates = ratedDramaMovies.reduce(function (acc, aFilm){
//   return acc + parseFloat(aFilm.rate);
// }, 0);

// console.log(sumDMRates);

// function dramaMoviesRate (originalArray, rates){
//     var averageRate = rates/originalArray.length;
//   if (Number.isInteger(averageRate)=== true){
//     return averageRate;
//   } else {
//     return parseFloat(averageRate.toFixed(2));
...
// console.log(dramaMoviesRate(dramaMovies, sumDMRates));

// Order by time rating, in growing order

//This function creates an array of ratings
function numberifyRatings (myMovies){
  myMovies.forEach(function(movie){
    return parseFloat(movie.rate);
  })
};

console.log(numberifyRatings(movies));
// function orderByRating (myArray){
