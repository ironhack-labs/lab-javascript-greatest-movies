...
return accum + Number(eachMovie.rate);//number to place the rate in numbers
}, 0)
...
let totalDramaArray = dramaMovieArray.filter((eachMovie)=>{//filter is when you want a new array
...
let orderedMovies = newMovieArray.sort((a, b)=>{
  ...
  if(a.duration < b.duration){
      ...
      return 1;
  }else{ //this else only happenw if they have the same duration
    if(a.title < b.title){
        return -1;
