...
function ratesAverage(arr){
  var avg= 0; 
    arr.reduce(function(acc,val){ 
      var toNumber = parseFloat(val.rate)
      var sumRate = acc + toNumber
  ...
  function dramaMoviesRate(arr){
    ...
    var avg = filter.reduce(function(acc,val){
      var toNumber = parseFloat("0"+val.rate)
      return acc + toNumber 
    },0)
    
    
    avg = avg / filter.length;//return undefined if no drama movie
    avg = avg.toFixed(2);
...
var dramaMovies = arr.filter(function(val){return val.genre.includes("Drama")})
var stevenDrama = dramaMovies.filter(function(val){return val.director === 'Steven Spielberg'});

  if(stevenDrama.length === 0){
  return 'Steven Spielberg directed 0 drama movies!';
  } 
  if(stevenDrama.length === 1){
    return 'Steven Spielberg directed 1 drama movies!';
    } 
  if(stevenDrama.length === 2){
    return 'Steven Spielberg directed 2 drama movies!';
    }
  if(stevenDrama.length === 4){
    return 'Steven Spielberg directed 4 drama movies!';
    } 

stevenDrama = stevenDrama.toString();
...
function orderAlphabetically(arr){
  var justTitles = arr.map(function(val){
    return val.title
  });
  var sortArr = justTitles.sort()
  var smallArr = sortArr.slice(0, 20);
