...
var movies = [
  ...
  function orderByDuration(array){
    ...
    return a.duration - b.duration;
     ...
     minutesArray.forEach(function(x){console.log(x.duration)});
    console.log("la duracion es "+minutesArray[0].duration+"y la mas larga es "+minutesArray[minutesArray.length-1].duration);
    return minutesArray;
  }
  var ordenadas = orderByDuration(movies);
  turnHoursToMinutes(movies);
  
  function ratesAverage(arrayMov){
    var rateSum = arrayMov.reduce(function(acum,item){
     // console.log(acum);
      return acum+=parseInt(item.rate);
    },0);
    var numRound = rateSum/arrayMov.length;
  ...
  function dramaMoviesRate (array){
    var dramaMovies= array.filter(function(drama){
          var exist =0;
          drama.genre.forEach(function(item){if (item==="Drama"){exist+=1;}});
          //console.log(exist);
         return exist>0  });
