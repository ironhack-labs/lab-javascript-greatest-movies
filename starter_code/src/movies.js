...
var turnHoursToMinutes = function (movies) {
  return movies.map(function (movie) {
    var totalminutes = 0;

    if (movie.duration.indexOf("h") === -1) {
      var minutes = movie.duration.slice(0, movie.duration.indexOf("m"));
      ...
      var hours = movie.duration.slice(0, movie.duration.indexOf("h"));
      var minutes = movie.duration.slice(movie.duration.indexOf("h") + 2, movie.duration.indexOf("m"));
      totalminutes = parseInt(hours) * 60 + parseInt(minutes);
    }

    var movieClone = Object.assign({}, movie) //Clone the object
...
var ratesAverage = function (movies) {
  var sumOfRates = movies.reduce(function (acc, movie) {
    return acc + parseFloat(movie.rate);
  }, 0);
  var fixed = (sumOfRates / movies.length).toFixed(2);
...
var orderByDuration = function (movies) {
...
var howManyMovies = function (movies) {
  if (movies.length < 1) {
    return undefined;
  } else {
...
var orderAlphabetically = function (movies) {
  var titles = [];
  for (movie of movies) {
