...
const ratesAverage = (arr) => {
    const sum = arr.reduce((acc, cv) => {
        const newcv = parseFloat(cv.rate)
        parseFloat(acc)
        return acc + newcv
    }, 0)
    const rn = parseFloat((sum / movies.length).toFixed(2))
    console.log(rn)
} 
//TEST 
//ratesAverage(movies) 
...
const dramaMoviesRate = (arr) => {
  ...
  let newArr = arr.filter(element => element.genre.includes("Drama"))
  const sum = newArr.reduce((acc, cv) => {
    const newcv = parseFloat(cv.rate)
    parseFloat(acc)
    return acc + newcv
}, 0)
const rn = parseFloat((sum / newArr.length).toFixed(2))
console.log(rn)
  }
} 
//TEST
...
const howManyMovies = (arr) => {
  const filteredArray = arr.filter((element) => element.director.includes("Steven Spielberg") )
