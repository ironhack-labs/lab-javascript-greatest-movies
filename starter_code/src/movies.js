...
function howManyMovies(array){
    let newArray = array.filter(movie => movie.director.includes("Steven Spielberg") && movie.genre.includes("Drama"))
    if (!checkNull(newArray)) {return 0}
    return newArray.length
}
// Iteration 3: Alphabetic Order - Order by title and print the first 20 titles
function orderAlphabetically(array){
    newArray = JSON.parse(JSON.stringify(array))
    newArray.sort((a,b) => {
        if (a.title > b.title) {
            return 1
    ...
    const titles = newArray.map(a => a.title);
    if (titles.length > 20) {
        titles.splice(20)
    }
    return titles
...
function ratesAverage(array) {
    if (!checkNull(array)) {return 0}
    return Math.round((array.filter(movie => movie.rate !== undefined).reduce(function(total, movie){
        if (movie.rate >= 8){
...
function dramaMoviesRate(array){
    if (!checkNull(array)) {return 0}
    return ratesAverage(array.filter(movie => movie.genre.includes("Drama")))
}
...
function bestYearAvg(array) {
    ...
    let lastYear = newArray[0].year, average = [];
    let array2 = newArray.map(function getYear(checkArray){
        if (lastYear === checkArray.year) {
            average.push({year: checkArray.year, rate: checkArray.rate})
            //average.push(average[checkArray.year][checkArray.rate])
            //console.log(Object.keys("avg").length)
            lastYear = checkArray.year;
        } else {
            average.push([{year: checkArray.year, rate: checkArray.rate}])
            //console.log(Object.keys("avg").length)
            lastYear = checkArray.year;
    ...
    for (i=0;i<average.length;i++){
        ...
        average.map(function getAverage(array){
            //console.log(array)
    })}
    return average
}
