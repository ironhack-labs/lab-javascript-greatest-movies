...
console.log(parseFloat(movies[0].rate))

function ratesAverage (movies){
    var sumRatings =
        movies.reduce(function (accumulator, oneMovie){
            return accumulator + parseFloat(oneMovie.rate);
        },0);
    var average = sumRatings/movies.length;
...
function dramaMoviesRate (movies){
    ...
    if movies.genre[]
    //var average = sumRatings/sumDramaMovies;
    //return average.toFixed(2);
    return sumRatings;
}
...
function orderByRating (movies){
    ...
    movies.sort(function (movieA, movieB){
        ...
        //     if (movieA.rate === movieB.rate){
        //         movies.sort(function (movieA, movieB){
        //             movieA.title= movieA.title.toLowerCase();
        //             movieB.title = movieB.title.toLowerCase();
...
console.log(orderByRating(movies));
...
function howManyMovies (movies){
    ...
    movies.filter(function (oneMovie){
        for (j=0; j<movies.length; j++){
            if (oneMovie.director === 'Steven Spielberg' && oneMovie.genre[j] === 'Drama'){
                return oneMovie;
            }
        }
    });
...
console.log("Steven Spielberg directed " + howManyMovies(movies).length + " drama movies !");
