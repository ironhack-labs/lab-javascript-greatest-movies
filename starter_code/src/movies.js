...
const ratesAverage = (movies) => {
  ...
  const dramaMoviesRate = (movies) => {
    let dramaMovies = movies.filter(movie => movie.genre.includes('Drama')); // .includes will return true or false if it finds a match 
...
// Iteration 4: Steven Spielberg. The best? - How many movies did STEVEN SPIELBERG direct
  ...
  let howManyMovies = (movies) => {
    console.log(movies)
      let steven = movies.filter((item) => item.genre.includes('Drama') && item.director === "Steven Spielberg")
