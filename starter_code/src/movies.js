...
function turnHoursToMinutes(movies){
  return movies.map((movie, i, movies) => {
    if (typeof movie.duration == "number") {
      var hour = 0;
      var min = movie.duration;
    } else {
      var t = movie.duration.split("h ")
      if (t.length == 2) {
        var hour = parseInt(t[0])
        var min = parseInt((t[1].split("m"))[0])
      } else {
        if ((t[0].split("h"))[1] == "") {
          var hour = parseInt(t[0])
          var min = 0
          ...
          var hour = 0
          var min = parseInt((t[0].split("m"))[0])
        }
      }
    }
    var movieCopy = {...movie, duration: hour * 60 + min}
    return movieCopy
  });
}
...
function ratesAverage(movies){
  var ratings = movies.filter(movie => movie.rating !== 'undefined' ); 
  var rates = movies.map(movie => { return (movie.rate === '' ? 0 : parseFloat(movie.rate)) })
  var sum = rates.reduce((total, amount) => total + amount); 
  return Math.round((sum / ratings.length) * 100) / 100
}
// console.log("Average Movies: ", ratesAverage(movies))
...
function dramaMoviesRate(movies) {
  ...
  for (i in movies)
    if (movies[i].genre.includes("Drama"))
      dramas.push(movies[i])
  return (dramas.length === 0 ? undefined : ratesAverage(dramas))
}
// console.log("Average Dramas: ", dramaMoviesRate(movies))
...
function orderByDuration(movies) {
  ...
  if (a.duration < b.duration || ( a.duration == b.duration && a.title < b.title))
    return -1;
  else
    return 1;
  });
...
function howManyMovies(movies){
  ...
  var spielbergs = dramas.filter(drama => drama.director === 'Steven Spielberg'); 
  return ("Steven Spielberg directed " + spielbergs.length.toString() + " drama movies!")
}
...
function orderAlphabetically(movies) {
  var alpha = movies.sort(function(a,b){
  if (a.title.toUpperCase() < b.title.toUpperCase())
    return -1;
  else
    return 1;
  });
  return alphaTitles = alpha.map(movie => { return movie.title }).slice(0, 20);
}
...
function bestYearAvg(movies) {
  ...
  for(var i=0; i<movies.length; i++) {
    if( flags[movies[i].year]) continue;
    flags[movies[i].year] = true;
  ...
  for (y in years) {
    ...
    var count = 0
    for (m in movies)
      if (movies[m].year == years[y]) {
        yearSum += parseFloat(movies[m].rate);
        count ++;
      }
      if (yearSum/count > bestAveRate || (yearSum/count == bestAveRate && years[y] < years[bestYearIndex])) {
        bestAveRate = yearSum/count;
        bestYearIndex = y;
      }
  }
return "The best year was " + years[bestYearIndex] + " with an average rate of " + bestAveRate
}
...
// is this the way to read a text file in??
// const fs = require('fs')
// const movies = fs.readFileSync('data.txt', 'utf8')